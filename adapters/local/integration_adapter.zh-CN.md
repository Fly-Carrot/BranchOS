# 本地 Branch Builder Adapter

[English](integration_adapter.md) | [简体中文](integration_adapter.zh-CN.md)

Branch Builder 在 skill 层是可移植的。runtime-specific boot、memory 和 sync system 应该通过这样的本地 adapter 接入。

## 放置位置

在 runtime 已经加载项目上下文之后、专门能力分发之前运行 Branch Builder：
...
使用 JSON 中的 `branch_builder_summary` 查看任务开始时的分支地图；使用 `branch_builder_delta` 查看最终分支报告。
