# GitHub Intro Test：BranchOS 处理研究管线任务

[English](github_intro_test.md) | [简体中文](github_intro_test.zh-CN.md)

本文面向公开展示，说明 BranchOS 在一个 medium-to-complex 任务上如何运行。
...
BranchOS 不会一开始就直接写答案。它会先创建虚拟分支图：
...
在调用专门能力之前，BranchOS 会创建 branch packet。示例 packet 是：
...
- BranchOS 可以把复杂任务表示为 standing branches 和 working branches。
